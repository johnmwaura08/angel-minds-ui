...
export module AngelMindsHelperFunctions {
  ...
  export const passwordComplexity = (e: any): boolean =>
    validatePasswordComplexity(e.value);
  ...
  export const isValidArray = <T>(arr: T[] | undefined | null): boolean =>
  ...
  export function handleAxiosError(error: any) {
    if (axios.isAxiosError(error)) {
      ...
      toaster(ToastTypeEnum.Error, error?.response?.data?.message);
    } else {
      console.error(error);
  ...
  export function removeItem(array: any[], removeIdx: any) {
    return array.filter((_: any, idx: any) => idx !== removeIdx);
  }

  export function insertItem(array: any, item: any, insertIdx: number) {
    const newArray = [...array];
    newArray.splice(insertIdx, 0, item);
    return newArray;
  }

  export function reorderItem(array: string[], fromIdx: any, toIdx: any) {
